...
class SerialCom:
    ...
    def sendData(self, data):
        ...
        header = bytearray([self.start_byte, first_length_byte, second_length_byte])
    ...
    def receiveData(self, iterations = 999999):
        ...
        while self.ser.in_waiting and iterations:
            ...
            if just_read_int == self.start_byte:
                if self.in_message:
                    raise Exception('Received a start command when already in a message')
                self.in_message = True
                self.next_reserved = False
                self.write_pos = -2
            elif just_read_int == self.end_byte:
                if not self.in_message:
                    raise Exception('Received an end command when not in a message')
                elif self.write_pos == self.expected_data_length:
                    self.data_handler(*self.data)
                else:
                    raise Exception('Received end command prior to expected end of message')
                self.in_message = False
            elif self.in_message:
                if just_read_int == self.reserved_byte:
                    if self.next_reserved:
                        self.in_message = False 
                        raise Exception('Received back to back reserved byte flags')
                    ...
                    if self.next_reserved:
                        ...
                        if just_read_int >= len(self.reserved_dict):
                            raise IndexError('Received a reserved index out of range')
                            self.in_message = False
                        else:
                            just_read_int = self.reserved_dict[just_read_int]
                    
                    if self.write_pos >= len(self.data):
                        raise IndexError('Tried to write outside of data array')
                    else:
