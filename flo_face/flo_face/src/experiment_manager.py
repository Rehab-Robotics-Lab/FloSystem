...
dynamic_mode = ['chortling', 'cheeky', 'content',
                'laughing', 'smiling_hard', 'happy']
...
def change_face(name):
    ...
    if not resp.success:
        rospy.logerr(
            'couldn\'t get the face changed: {} when using key: {}'.format(resp.info, name))
    elif resp.success:
    ...
    while not rospy.is_shutdown():
        if select.select([sys.stdin], [], [], 0)[0] == [sys.stdin]:
            ...
            if new_input == 's':
                ...
                cycle_idx = 0
                print(
                    'Set to cycle through all faces, press the n key to go to the next face')
