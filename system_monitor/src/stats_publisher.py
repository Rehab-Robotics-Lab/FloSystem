...
class StatsPublisher(object):
    ...
    def __init__(self):
        ...
        self.cpu_stats_pub = rospy.Publisher('cpu_stats', CPUutil,queue_size=20)
        self.mem_stats_pub = rospy.Publisher('mem_stats', MEMutil,queue_size=20)
        self.net_stats_pub = rospy.Publisher('net_stats', NETstats,queue_size=20)
        self.hdd_stats_pub = rospy.Publisher('hdd_stats', HDDutil,queue_size=20)
    ...
    def read_net(self,event):
        ...
        if net_stats:
            ...
            rospy.logerr('could not get network stats')
    ...
    def get_net_strength():
        try:
            ...
            if not output:
                if err:
                    rospy.logerr('no output recieved while checking net strength, got error:\n%s',err)
                else:
                    rospy.logerr('no output recieved while checking net strength')
                
            msg = output.decode('utf-8')
            lqv = re.split('/',re.search('(?<=Link Quality=)[0-9/]*',msg).group(0))
            link_quality = int(lqv[0])/int(lqv[1])
            signal_level = int(re.search('(?<=Signal level=)[0-9\-]*',msg).group(0))
